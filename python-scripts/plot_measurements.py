...
if matched_fc_keys:
    ...
    ax.set_ylabel("Prover time (s)", fontsize=16)
    ax.set_title("Our System vs zkVM: Prover Time", fontsize=18)
...
if matched_fc_keys:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([fc_label(k) for k in matched_fc_keys], fontsize=8)
    ax.set_ylabel("Prover RAM (GB)", fontsize=16)
    ax.set_title("Our System vs zkVM: Prover RAM", fontsize=18)
    ax.legend(fontsize=18)
    fig.tight_layout()
